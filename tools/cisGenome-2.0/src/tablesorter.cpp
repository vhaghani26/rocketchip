...
{
   if(item1.chr < item2.chr)
	   return true;

	if(item1.chr == item2.chr && item1.coord < item2.coord)
		return true;
...
bool tablefile_sort(string infile, string outfile)
{
	...
	while(fgets(strLine, LONG_LINE_LENGTH, fpIn) != NULL)
	{
		...
		while(chp2 != NULL)
		{
			*chp2 = '\0';
			...
			{
				newitem.intensities.push_back(atof(chp1));
			}
	...
	{
		fprintf(fpOut, "%s", table_data[i].chr.c_str());
		...
		{
			fprintf(fpOut, "\t%f", table_data[i].intensities[j]);
		}

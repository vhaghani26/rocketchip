...
int ESClustGetPromoter(int argv, char **argc)
{
	...
	strcpy(strGenomePath, "/Volumes/FTP/genomes/mouse/mm6/");
	strcpy(strCSPath, "/Volumes/FTP/genomes/mouse/mm6/conservation/genomelab/cs/");
	...
	while(fgets(strLine, LONG_LINE_LENGTH, fpIn)!=NULL)
	{
		...
		if(pRefGene->chStrand == '+')
		{
			...
			if(nEnd >= pRefGene->nTxEnd)
				nEnd = pRefGene->nTxEnd-1;
		...
		else
		{
			...
			if(nStart <= pRefGene->nTxStart)
				nStart = pRefGene->nTxStart+1;
			/* if(pRefGene->nExonCount >= 1)
			{

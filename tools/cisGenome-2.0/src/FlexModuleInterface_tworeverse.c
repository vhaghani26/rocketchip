			...
			int nMotifNum)
{
	...
	double dScore = 0.0;
	int ni,nk;
	struct INTMATRIX *pSiteP;
	struct INTMATRIX *pSiteN;
	...
	/* ------------------------------------ */
	pSiteP = NULL;
	pSiteP = CreateIntMatrix(1, nMotifNum);
	if(pSiteP == NULL)
	{
	...
	pSiteN = NULL;
	pSiteN = CreateIntMatrix(1, nMotifNum);
	if(pSiteN == NULL)
	{
		printf("Error: FlexModule_ComputeModuleScore, cannot allocate memory for computing module score!\n");
		exit(EXIT_FAILURE);
	...
	for(ni=0; ni<nContextNum; ni++)
	{
		nk = pMotifId[ni]-1;
		if(pStrand[ni] == 0)
			pSiteP->pMatElement[nk] += 1;
		else
			pSiteN->pMatElement[nk] += 1;
	}

	if(nAddone == 1)
	{
		if(nAddStrand == 0)
			pSiteP->pMatElement[nAddMotifId-1] += 1;
		else
			pSiteN->pMatElement[nAddMotifId-1] += 1;
	}
	
	if( (pSiteP->pMatElement[0] > 0) && (pSiteN->pMatElement[1] > 0) )
		dScore = 1.0;
	else if( (pSiteP->pMatElement[1] > 0) && (pSiteN->pMatElement[0] > 0) )
		dScore = 1.0;
	else
		dScore = 0.0;

	DestroyIntMatrix(pSiteP);
	DestroyIntMatrix(pSiteN);
